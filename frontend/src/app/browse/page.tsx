...
export default function BrowsePage() {
  ...
  const fetchAssets = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/assets${query}`
      );
  ...
  return (
    <div className="min-h-screen bg-[#050505] relative overflow-hidden">
      ...
      <div
        className="absolute bottom-0 right-1/4 w-96 h-96 bg-cyan-600/20 rounded-full mix-blend-screen filter blur-[100px] opacity-30 animate-pulse"
        ...
        <div className="container-custom relative z-10">
          <div className="max-w-4xl mx-auto text-center mb-12 animate-fade-in px-4">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
              Discover Premium{" "}
              <span className="gradient-text">Game Assets</span>
            </h1>
            <p className="text-base sm:text-lg md:text-xl text-gray-400 mb-8 max-w-2xl mx-auto">
              Browse our curated collection of high-quality assets. From 3D
              models to sound effects, find everything you need.
            </p>
      ...
      <section className="pb-20">
        <div className="container-custom relative z-10">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <aside
              className={`w-full lg:w-72 shrink-0 space-y-6 animate-slide-up ${
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 mt-1">
                    {loading
                      ? "Loading..."
                      : `${assets.length} results • Page ${currentPage}`}
                  </p>
                </div>
              </div>
              ...
              {loading ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div
                      key={i}
                      className="h-80 bg-linear-to-br from-[#111] to-[#0a0a0a] rounded-2xl animate-pulse border border-white/5 relative overflow-hidden"
                    >
                  ...
                  {pagination && (pagination.next || pagination.prev) && (
                    <div className="mt-12 flex items-center justify-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        {[...Array(Math.ceil(totalAssets / 12))].map(
                          (_, idx) => {
                            const pageNum = idx + 1;
                            if (
                              pageNum === 1 ||
